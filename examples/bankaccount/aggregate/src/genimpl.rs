...
impl AggregateService for BankaccountAggregateImpl {
    async fn handle_command(&self, _ctx: &Context, arg: &StatefulCommand) -> RpcResult<EventList> {
        ...
        match arg.command_type.as_str() {
            CreateAccount::TYPE => BankaccountAggregate::handle_create_account(
                self,
                deserialize_json(&arg.payload)?,
                state,
            ),

            ReserveFunds::TYPE => BankaccountAggregate::handle_reserve_funds(
                self,
                deserialize_json(&arg.payload)?,
                state,
            ),

            ReleaseReservedFunds::TYPE => BankaccountAggregate::handle_release_reserved_funds(
                self,
                deserialize_json(&arg.payload)?,
                state,
            ),

            WithdrawFunds::TYPE => BankaccountAggregate::handle_withdraw_funds(
                self,
                deserialize_json(&arg.payload)?,
                state,
            ),

            DepositFunds::TYPE => BankaccountAggregate::handle_deposit_funds(
                self,
                deserialize_json(&arg.payload)?,
                state,
            ),

            RequestWireTransfer::TYPE => BankaccountAggregate::handle_request_wire_transfer(
                self,
                deserialize_json(&arg.payload)?,
                state,
            ),
            ...
            WithdrawReservedFunds::TYPE => BankaccountAggregate::handle_withdraw_reserved_funds(
                self,
                deserialize_json(&arg.payload)?,
                state,
            ),
    ...
    async fn apply_event(&self, _ctx: &Context, arg: &EventWithState) -> RpcResult<StateAck> {
        ...
        Ok(match arg.event.event_type.as_str() {
            AccountCreated::TYPE => BankaccountAggregate::apply_account_created(
                self,
                deserialize_json(&arg.event.payload)?,
                state,
            )?,
            WireFundsReserved::TYPE => BankaccountAggregate::apply_wire_funds_reserved(
                self,
                deserialize_json(&arg.event.payload)?,
                state,
            )?,
            FundsWithdrawn::TYPE => BankaccountAggregate::apply_funds_withdrawn(
                self,
                deserialize_json(&arg.event.payload)?,
                state,
            )?,
            WireFundsReleased::TYPE => BankaccountAggregate::apply_wire_funds_released(
                self,
                deserialize_json(&arg.event.payload)?,
                state,
            )?,
            ...
            WireTransferRequested::TYPE => BankaccountAggregate::apply_wire_transfer_requested(
                self,
                deserialize_json(&arg.event.payload)?,
                state,
            )?,
            FundsDeposited::TYPE => BankaccountAggregate::apply_funds_deposited(
                self,
                deserialize_json(&arg.event.payload)?,
                state,
            )?,
            ReservedFundsWithdrawn::TYPE => BankaccountAggregate::apply_reserved_funds_withdrawn(
                self,
                deserialize_json(&arg.event.payload)?,
                state,
            )?,
