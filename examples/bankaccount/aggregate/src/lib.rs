use eventsourcing::*;
use genimpl::BankaccountAggregateImpl;
...
impl BankaccountAggregate for BankaccountAggregateImpl {
    ...
    fn apply_funds_deposited(
        ...
        let Some(old_state) = state else {
            ...
            return Ok(StateAck::error("Account does not exist", None));
        };
        let state = BankaccountAggregateState {
...
impl StateAck {
    fn ok(state: Option<BankaccountAggregateState>) -> StateAck {
        StateAck {
            succeeded: true,
            error: None,
            state: state
    ...
    fn error(msg: &str, state: Option<BankaccountAggregateState>) -> StateAck {
        StateAck {
            succeeded: false,
